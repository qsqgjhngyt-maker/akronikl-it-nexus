...
function findProjectMember(access,subjectId){return(access?.members||[]).find(member=>member?.subjectId===subjectId&&member?.status!=='revoked')||null}
function evaluateProjectAccess(access,{subjectId,path='/',action='view'}={}){
...
function corsHeaders(request,env){
  ...
  return{'access-control-allow-origin':origin,'access-control-allow-methods':'GET,PUT,POST,DELETE,OPTIONS','access-control-allow-headers':'authorization,content-type,if-match,x-nexus-bootstrap-secret','access-control-max-age':'86400','vary':'Origin'};
}
...
async function sha256(text){const data=new TextEncoder().encode(String(text??''));const digest=await crypto.subtle.digest('SHA-256',data);return[...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join('')}
function randomToken(){const bytes=new Uint8Array(32);crypto.getRandomValues(bytes);let binary='';for(const b of bytes)binary+=String.fromCharCode(b);return`nxk_${btoa(binary).replaceAll('+','-').replaceAll('/','_').replace(/=+$/,'')}`}
async function sameSecret(a,b){if(!a||!b)return false;const[ha,hb]=await Promise.all([sha256(a),sha256(b)]);return ha===hb}
...
async function authenticate(request,env){
  ...
  if(mode==='dev'){
    ...
    return{subjectId:match[1],displayName:match[1],authMode:'dev'};
  }
  if(mode==='nexus-token'){
    const raw=request.headers.get('authorization')||'';const match=/^Bearer\s+(nxk_[A-Za-z0-9_-]{24,})$/i.exec(raw);
    if(!match)throw new ApiError(401,'UNAUTHORIZED','Nexus account token required.');
    const tokenHash=await sha256(match[1]);
    const row=await env.DB.prepare(`SELECT s.id AS subject_id,s.display_name,t.id AS token_id FROM account_tokens t JOIN account_subjects s ON s.id=t.subject_id WHERE t.token_hash=?1 AND t.status='active' AND s.status='active' LIMIT 1`).bind(tokenHash).first();
    if(!row)throw new ApiError(401,'INVALID_TOKEN','Nexus account token is invalid or revoked.');
    env.DB.prepare('UPDATE account_tokens SET last_used_at=?1 WHERE id=?2').bind(now(),row.token_id).run().catch(()=>{});
    return{subjectId:row.subject_id,displayName:row.display_name,tokenId:row.token_id,authMode:'nexus-token'};
  }
  throw new ApiError(503,'AUTH_ADAPTER_REQUIRED',`Unsupported AUTH_MODE: ${mode}.`);
}

async function bootstrapAccount(request,env){
  if(String(env.AUTH_MODE||'disabled')!=='nexus-token')throw new ApiError(409,'BOOTSTRAP_MODE_MISMATCH','Set AUTH_MODE=nexus-token before account bootstrap.');
  const secret=String(env.BOOTSTRAP_SECRET||'');
  if(!secret)throw new ApiError(503,'BOOTSTRAP_SECRET_MISSING','BOOTSTRAP_SECRET Worker secret is not configured.');
  const provided=request.headers.get('x-nexus-bootstrap-secret')||'';
  if(!(await sameSecret(secret,provided)))throw new ApiError(401,'BOOTSTRAP_UNAUTHORIZED','Bootstrap secret is invalid.');
  const count=await env.DB.prepare('SELECT COUNT(*) AS n FROM account_subjects').first();
  if(Number(count?.n||0)>0)throw new ApiError(409,'BOOTSTRAP_CLOSED','The first Nexus account already exists; bootstrap is closed.');
  const payload=await bodyJson(request);const displayName=String(payload?.displayName||'Nexus Owner').trim().slice(0,120)||'Nexus Owner';
  const subjectId=id('user'),tokenId=id('token'),token=randomToken(),tokenHash=await sha256(token),stamp=now();
  await env.DB.batch([
    env.DB.prepare('INSERT INTO account_subjects (id,display_name,status,created_at,updated_at) VALUES (?1,?2,?3,?4,?5)').bind(subjectId,displayName,'active',stamp,stamp),
    env.DB.prepare('INSERT INTO account_tokens (id,subject_id,token_hash,label,status,created_at) VALUES (?1,?2,?3,?4,?5,?6)').bind(tokenId,subjectId,tokenHash,'bootstrap owner token','active',stamp)
  ]);
  return{ok:true,subject:{id:subjectId,displayName},token,warning:'This token is shown once. Store it securely; the server stores only its SHA-256 hash.'};
}
async function health(env){
  let bootstrapOpen=null;
  if(env.DB&&String(env.AUTH_MODE||'')==='nexus-token'){
    try{const row=await env.DB.prepare('SELECT COUNT(*) AS n FROM account_subjects').first();bootstrapOpen=Number(row?.n||0)===0}catch{bootstrapOpen=null}
  }
  return{ok:true,service:'akronikl-nexus-sync',version:VERSION,authMode:String(env.AUTH_MODE||'disabled'),d1:Boolean(env.DB),r2:Boolean(env.SNAPSHOTS),bootstrapOpen};
}
async function me(actor){return{subject:{id:actor.subjectId,displayName:actor.displayName||actor.subjectId},authMode:actor.authMode}}
...
async function requireAction(env,project,subjectId,action,path='/'){
  ...
  if(!verdict.allowed)throw new ApiError(403,'FORBIDDEN',`Action ${action} is not allowed for ${path}.`,{reason:verdict.reason});return verdict;
}
...
function changedFiles(beforeProject,afterProject){
  ...
  for(const path of paths){if(!before.has(path))changes.push({path,kind:'create'});else if(!after.has(path))changes.push({path,kind:'delete'});else if(before.get(path)!==after.get(path))changes.push({path,kind:'edit'})}return changes;
}
...
async function putProject(env,actor,projectId,payload){
  ...
  let workspaceId=incoming.access?.workspaceId||incoming.workspaceId;let ownerId=incoming.access?.ownerId||actor.subjectId;let previousSnapshot=null;let fileChanges=[];
  if(existing){
    ...
    fileChanges=changedFiles(previousSnapshot,incoming);for(const change of fileChanges)await requireAction(env,existing,actor.subjectId,change.kind,change.path);
    const metadataChanged=!previousSnapshot||String(previousSnapshot.title||'')!==String(incoming.title||'')||String(previousSnapshot.languageId||'')!==String(incoming.languageId||'');if(metadataChanged)await requireAction(env,existing,actor.subjectId,'edit','/');
  }else{
  ...
  const revision=(existing?Number(existing.current_revision):0)+1,stamp=now(),serialized=JSON.stringify(incoming),hash=await sha256(serialized),key=snapshotKey(workspaceId,projectId,revision);
...
async function route(request,env){
  ...
  if(path==='/api/v1/health')return json(await health(env));
  if(path==='/api/v1/bootstrap'&&request.method==='POST')return json(await bootstrapAccount(request,env),201);
  if(!path.startsWith('/api/v1/'))throw new ApiError(404,'NOT_FOUND','Route not found.');
  const actor=await authenticate(request,env);
  if(path==='/api/v1/me'&&request.method==='GET')return json(await me(actor));
...
export default{async fetch(request,env){try{return withCors(await route(request,env),request,env)}catch(error){const status=Number(error?.status||500),body={error:{code:error?.code||'INTERNAL_ERROR',message:error?.message||'Internal error',...(error?.details?{details:error.details}:{})}};return withCors(json(body,status),request,env)}}};
