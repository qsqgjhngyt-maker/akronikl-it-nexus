...
export function findProjectMember(access,subjectId){
  return (access?.members||[]).find(member=>member?.subjectId===subjectId&&member?.status!=='revoked')||null;
}
...
export function evaluateProjectAccess(access,{subjectId,path='/',action='view'}={}){
  ...
  const member=findProjectMember(access,subjectId);
  if(!member)return{allowed:false,reason:'not-a-member',role:null,matchedPolicies:[]};
  const role=member.role||'viewer';
  const grants=ROLE_GRANTS[role]||[];
  let allowed=grants.includes('*')||grants.includes(action);
...
export function createPrivateAccessModel({ownerId,workspaceId,createdAt=new Date().toISOString()}={}){
  ...
  return{
    ...
    visibility:'private',
    ownerId,
    members:[{subjectId:ownerId,role:'owner',status:'active',addedAt:createdAt,addedBy:ownerId}],
    policies:[]
  };
}

export function accessSummary(access){
  const members=(access?.members||[]).filter(x=>x?.status!=='revoked');
  return{
    workspaceId:access?.workspaceId||null,
    workspaceKind:access?.workspaceKind||'personal',
    visibility:access?.visibility||'private',
    ownerId:access?.ownerId||null,
    memberCount:members.length,
    guestCount:members.filter(x=>x.subjectId!==access?.ownerId).length,
    policyCount:(access?.policies||[]).length
  };
}
