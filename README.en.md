...
**v0.1.7-alpha.1.3 / Project VFS Paths + Editor Resize + Session Resume Hotfix.** Project Studio now mounts nested `src/`, `include/`, and `tests/` paths correctly into the Clang/WASI virtual filesystem; normal Run builds exclude test translation units. Code Studio grows and shrinks with source size, while exact page/caret/editor-scroll positions are scoped to the current session only.
...
`v0.1.7-alpha.1.3` keeps the proven multi-file Clang/WASI baseline and hardens Project Studio nested VFS mapping, Code Studio bidirectional auto-resize, and session-only positional resume while preserving persistent code, projects, checkpoints, and progress.
