...
**v0.1.2-alpha.1 / Benchmark Lesson 01.** Рабочий baseline v0.1.1-alpha.3 сохранён. Курс C++ остаётся первым reference-course, а тема `cpp.first-cpp-program` стала первым полным эталонным уроком Nexus: глубокая теория, внутренняя модель toolchain, построчный разбор, эксперименты, ошибки «симптом → причина → исправление», лаборатория, Skill Graph и полноценные RU/EN тела урока. Остальные 39 тем пока используют baseline v7.
...
Текущий исполняемый релиз: [`RELEASE_v0.1.2-alpha.1.md`](RELEASE_v0.1.2-alpha.1.md). История изменений: [`CHANGELOG_v0.1.2-alpha.1.md`](CHANGELOG_v0.1.2-alpha.1.md).
