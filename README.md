...
**v0.1.2-alpha.2 / Benchmark Lesson 01 · Standard 1.1.** Курс C++ является первым активным курсом Nexus. Тема `cpp.first-cpp-program` усилена до эталона: фундаментальная теория, расширенный словарь терминов и сокращений, модель toolchain/runtime, диагностика, лабораторная работа, Skill Graph и встроенная Nexus Sandbox. Пользовательский учебный интерфейс больше не показывает отсылки к исходному PDF; provenance хранится отдельно. RU/EN используют один stable ID и один прогресс.
...
Текущий исполняемый релиз: [`RELEASE_v0.1.2-alpha.2.md`](RELEASE_v0.1.2-alpha.2.md). История изменений: [`CHANGELOG_v0.1.2-alpha.2.md`](CHANGELOG_v0.1.2-alpha.2.md).
