# Requirements Traceability Matrix — v0.1.7-alpha.2.3

| Requirement | Status | Architecture/Component | Verification | Baseline source |
| --- | --- | --- | --- | --- |
...
Для `IMPLEMENTED` requirement в следующих WORK-пакетах RTM должна быть усилена до точного пути `code → test → evidence`. Для `PLANNED/RESEARCH` запрещено указывать фиктивный PASS.
