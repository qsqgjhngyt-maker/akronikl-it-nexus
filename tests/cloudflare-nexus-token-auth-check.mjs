...
class Stmt{
  constructor(sql){this.sql=sql;this.args=[]}bind(...args){this.args=args;return this}
  async first(){
    if(this.sql.includes('COUNT(*) AS n FROM account_subjects'))return{n:state.subjects.size};
    if(this.sql.includes('FROM account_tokens t JOIN account_subjects s')){
  ...
  async run(){
    if(this.sql.startsWith('INSERT INTO account_subjects')){const[id,display,status,created,updated]=this.args;state.subjects.set(id,{id,display_name:display,status,created_at:created,updated_at:updated});return{success:true}}
    if(this.sql.startsWith('INSERT INTO account_tokens')){const[id,subject_id,token_hash,label,status,created_at]=this.args;state.tokens.set(id,{id,subject_id,token_hash,label,status,created_at});return{success:true}}
    if(this.sql.startsWith('UPDATE account_tokens SET last_used_at'))return{success:true};
    return{success:true};
  }
}
const DB={prepare:sql=>new Stmt(sql),batch:async stmts=>{for(const stmt of stmts)await stmt.run();return stmts.map(()=>({success:true}))}};
const env={DB,SNAPSHOTS:{},AUTH_MODE:'nexus-token',ENVIRONMENT:'production',BOOTSTRAP_SECRET:'a-very-long-bootstrap-secret-123456789',ALLOWED_ORIGIN:'https://example.github.io'};
