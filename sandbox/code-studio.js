...
export function bindCodeStudio({editor,locale='ru',languageId='cpp',fileName='main.cpp',files=null,activeFile=null,viewState=null,sessionKey=null,onWorkspaceChange=()=>{}}={}){
  ...
  const SESSION_VIEW_PREFIX='akronikl:code-studio:view:v1:';const resolvedSessionKey=sessionKey?`${SESSION_VIEW_PREFIX}${String(sessionKey)}`:null;const sessionStore=(()=>{try{return globalThis.sessionStorage||null}catch{return null}})();
  ...
  const loadSessionViewState=()=>{if(!resolvedSessionKey||!sessionStore)return null;try{return JSON.parse(sessionStore.getItem(resolvedSessionKey)||'null')}catch{return null}};
  const saveSessionViewState=value=>{if(!resolvedSessionKey||!sessionStore)return;try{sessionStore.setItem(resolvedSessionKey,JSON.stringify(value))}catch{}};
  const initialViewState=resolvedSessionKey?(loadSessionViewState()||null):viewState;
  for(const [path,value] of Object.entries(initialViewState?.files||{})){const safe=normalizeWorkspacePath(path),length=workspace.getFile(safe)?.content?.length;if(workspace.hasFile(safe))fileViews.set(safe,sanitizeView(value,length??0))}
  ...
  const studioSnapshot=()=>({...workspace.snapshot()});
  const notifyWorkspaceChange=type=>onWorkspaceChange(studioSnapshot(),type);
  const scheduleViewPersistence=()=>{clearTimeout(viewTimer);viewTimer=setTimeout(()=>{rememberView();saveSessionViewState(serializedViewState());notifyWorkspaceChange('view-state')},180)};
  ...
  const resetWorkspace=(nextFiles,nextActive=workspace.entryFile,{emit=true}={})=>{clearTimeout(historyTimer);clearTimeout(viewTimer);histories.clear();fileViews.clear();saveSessionViewState({version:1,files:{}});staticDiagnostics=[];runtimeDiagnostics=[];workspace.replaceFiles(nextFiles,{activeFile:nextActive});const file=workspace.getFile();applyingHistory=true;editor.value=file?.content??'';applyingHistory=false;restoreEditorView(workspace.activeFile);historyFor(workspace.activeFile);renderFileChrome();syncVisuals();renderProblems();if(emit)emitInput('insertReplacementText');return studioSnapshot()};
  ...
  const api={workspace,sync:syncVisuals,jumpTo,setStaticDiagnostics,setRuntimeDiagnostics,clearRuntimeDiagnostics,setValue:(value,options={})=>applyEditorValue(value,null,null,options),setWorkspace:resetWorkspace,loadDemoProject,switchFile,undo,redo,openSearch,closeSearch,runtimeRequest:stdin=>workspace.runtimeRequest(stdin),getFiles:()=>workspace.runtimeFiles(),getEntrySource:()=>workspace.getFile(workspace.entryFile)?.content??'',getActiveFile:()=>workspace.activeFile,getSnapshot:()=>studioSnapshot(),recalculateEditorHeight,getSessionViewState:()=>serializedViewState()};editor.__nexusCodeStudio=api;historyFor(workspace.activeFile);restoreEditorView(workspace.activeFile);renderFileChrome();syncVisuals();renderProblems();notifyWorkspaceChange('init');return api;
}
