...
export function bindCodeStudio({editor,locale='ru',languageId='cpp',fileName='main.cpp',files=null,activeFile=null,onWorkspaceChange=()=>{}}={}){
  ...
  const histories=new Map();let historyTimer=null,applyingHistory=false,staticDiagnostics=[],runtimeDiagnostics=[],searchIndex=-1;
  ...
  const switchFile=(path,{focus=true,emit=true}={})=>{
    const safe=normalizeWorkspacePath(path);if(!workspace.hasFile(safe))return null;
    clearTimeout(historyTimer);syncWorkspace();pushHistory();workspace.setActiveFile(safe);const file=workspace.getFile(safe);historyFor(safe);applyingHistory=true;editor.value=file?.content??'';editor.setSelectionRange(0,0);applyingHistory=false;editor.scrollTop=0;editor.scrollLeft=0;searchIndex=-1;if(searchCount)searchCount.textContent='0/0';renderFileChrome();syncVisuals();if(focus)editor.focus();if(emit)editor.dispatchEvent(new CustomEvent('nexus-workspace-active',{bubbles:true,detail:{path:safe}}));return file;
  };
  ...
  const addFile=()=>{const name=prompt(en?'New file name':'Имя нового файла','NewFile.cpp');if(!name)return;try{const safe=normalizeWorkspacePath(name);const initial=/\.(?:h|hpp|hh|hxx)$/i.test(safe)?'#pragma once\n':'';workspace.addFile(safe,initial);histories.delete(safe);switchFile(safe)}catch(error){alert(error.message)}};
  const renameFile=path=>{const next=prompt(en?'Rename file':'Новое имя файла',path);if(!next||next===path)return;try{clearTimeout(historyTimer);syncWorkspace();workspace.renameFile(path,next);const oldHistory=histories.get(path);histories.delete(path);if(oldHistory)histories.set(normalizeWorkspacePath(next),oldHistory);renderFileChrome();syncVisuals()}catch(error){alert(error.message)}};
  const removeFile=path=>{if(!confirm(en?`Delete ${path}?`:`Удалить ${path}?`))return;try{clearTimeout(historyTimer);syncWorkspace();const wasActive=workspace.activeFile===normalizeWorkspacePath(path);workspace.removeFile(path);histories.delete(normalizeWorkspacePath(path));if(wasActive)switchFile(workspace.activeFile,{emit:false});renderFileChrome();syncVisuals();editor.dispatchEvent(new CustomEvent('nexus-workspace-active',{bubbles:true,detail:{path:workspace.activeFile}}))}catch(error){alert(error.message)}};
  const resetWorkspace=(nextFiles,nextActive=workspace.entryFile,{emit=true}={})=>{clearTimeout(historyTimer);histories.clear();staticDiagnostics=[];runtimeDiagnostics=[];workspace.replaceFiles(nextFiles,{activeFile:nextActive});const file=workspace.getFile();applyingHistory=true;editor.value=file?.content??'';editor.setSelectionRange(0,0);applyingHistory=false;historyFor(workspace.activeFile);renderFileChrome();syncVisuals();renderProblems();if(emit)emitInput('insertReplacementText');return workspace.snapshot()};
  ...
  workspace.subscribe(({type,snapshot})=>{if(type!=='file-updated')renderFileChrome();onWorkspaceChange(snapshot,type)});
  const api={workspace,sync:syncVisuals,jumpTo,setStaticDiagnostics,setRuntimeDiagnostics,clearRuntimeDiagnostics,setValue:(value,options={})=>applyEditorValue(value,null,null,options),setWorkspace:resetWorkspace,loadDemoProject,switchFile,undo,redo,openSearch,closeSearch,runtimeRequest:stdin=>workspace.runtimeRequest(stdin),getFiles:()=>workspace.runtimeFiles(),getEntrySource:()=>workspace.getFile(workspace.entryFile)?.content??'',getActiveFile:()=>workspace.activeFile,getSnapshot:()=>workspace.snapshot()};editor.__nexusCodeStudio=api;historyFor(workspace.activeFile);renderFileChrome();syncVisuals();renderProblems();onWorkspaceChange(workspace.snapshot(),'init');return api;
}
